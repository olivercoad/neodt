...
for (const implementation of builtInAdapters)
  implementation.run(({ adapter, date, zone: nativeZone }) => {
    describe(implementation.name, () => {
      const zone = nativeZone("America/New_York");
      it("reuses one adapter formatter for different dates and both sides of a DST fold", () => {
        const values = [
          "2026-11-01T01:30-04:00",
          "2026-11-01T01:30-05:00",
          "2026-12-01T10:15-05:00",
        ].map((text) => calendarDate(adapter, date(text, "America/New_York")));
        const construct = vi.spyOn(adapter, "createFormatter");
        try {
      ...
      it("formats fields and fractional seconds at a quarter-hour offset", () => {
        const value = date("2026-09-23T12:34:56.789+05:45", "Asia/Kathmandu");
      ...
      it("keys by locale and option values, including mutated option objects", () => {
        ...
        const value = date("2026-12-01T15:00-05:00", "America/New_York");
        expect(part(twelveHour.formatToParts(value), "hour")).toBe("03");
        expect(part(initial.formatToParts(value), "hour")).toBe("15");
      });

      it("separates zones and adapter identities", () => {
        const first = formatterFor(adapter, zone, "en-US", options);
        expect(formatterFor(adapter, nativeZone("UTC"), "en-US", options)).not.toBe(first);
        expect(formatterFor({ ...adapter }, zone, "en-US", options)).not.toBe(first);
      });

      it("retains at most 32 entries per adapter and evicts the least recently used", () => {
        // Vary option values, so this also works with libraries whose fixed-offset databases are finite.
        const format = (index: number) =>
          formatterFor(adapter, zone, `en-US-x-cache${index}`, options);
...
describe("library-specific formatting", () => {
