...
for (const adapterName of libraries.map(({ id }) => id)) {
  test.describe(adapterName, () => {
    ...
    const goto = async (page: Page, path: string) => {
      ...
      url.pathname = `/${adapterName}/`;
      url.searchParams.set("fixture", "layout");
      await page.goto(url.pathname + url.search);
      await expect(page.locator("html")).toHaveAttribute("data-adapter", adapterName);
    };

    for (const locale of ["en-GB", "en-US", "de-DE", "ja-JP"]) {
      test(`${locale}: resize, focus and natural entry preserve the chosen rows`, async ({
        page,
      }) => {
        await goto(page, `/?locale=${locale}&offset`);
        ...
        for (const offset of [false, true]) {
          ...
          await expect
            .poll(() =>
              content(page).evaluate((el) => {
                ...
                const timezone = el
                  .querySelector(".datetime-neo__timezone")
                  ?.getBoundingClientRect();
                const offsetHeight =
                  el.querySelector(".datetime-neo__timezone-minutes")?.getBoundingClientRect()
                    .height ?? 0;
                const expectedTop =
          ...
          const excessHeight = await content(page).evaluate((el) => {
            ...
            return (
              ...
              Math.max(editorHeight, actionsHeight + offsetHeight)
            );
          });
    ...
    for (const zone of ["UTC", "Asia/Kolkata", "Asia/Kathmandu"]) {
      test(`${zone}: only whole-hour offset minutes collapse`, async ({ page }) => {
        await goto(page, `/?width=420&offset&zone=${zone}`);
    ...
    test("empty placeholders appear on focus and typing emits a date", async ({ page }) => {
      ...
      await expect(page.locator("output")).toHaveText("2026-08-18T09:00+10:00");
    });

    test("custom metrics align completion text and preserve wrapped row height", async ({
      page,
    }) => {
      await goto(page, "/?metrics&width=200");
      await rows(page, true);
      const height = (await box(control(page))).height;
      await content(page).getByRole("spinbutton").first().focus();
      await page.keyboard.press("@");
    ...
    for (const state of ["readonly", "disabled"]) {
      test(`${state}: actions are removed and keyboard editing is unavailable`, async ({
        page,
      }) => {
        await goto(page, `/?state=${state}&offset`);
    ...
    for (const width of [420, 210]) {
      test(`clicking control gaps repeatedly focuses the first segment at width ${width}`, async ({
        ...
        await goto(page, `/?offset&width=${width}`);
    ...
    for (const width of [420, 210]) {
      test(`value separators focus the closest segment on mouse-down at width ${width}`, async ({
        ...
        await goto(page, `/?locale=en-GB&offset&width=${width}`);
        await rows(page, width === 210);
        await control(page).hover();
        ...
        for (const size of [11, 13, 20]) {
          ...
          for (const width of [264, 140]) {
            ...
            for (const value of ["11:11", ""]) {
              ...
              if (!value) {
                const placeholder = content(page).locator(
                  ".datetime-neo__natural-ghost--placeholder",
                );
