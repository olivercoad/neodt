...
const date = (value: string, zone = "UTC") => CalendarDate.fromISO(value, { zone })!;
...
describe("shared calendar", () => {
  ...
  it("handles Gregorian century leap-year rules and clamps month changes", () => {
    expect(CalendarDate.fromISO("1900-02-29")).toBeUndefined();
    expect(CalendarDate.fromISO("2000-02-29")).toBeDefined();
  ...
  it("formats fixed offsets including quarter-hour zones", () => {
    ...
    expect(value.setZone("UTC").toLocalValue()).toBe("2025-12-31T18:15");
  ...
  it("rejects invalid ISO dates and offsets rather than rolling over", () => {
    for (const value of [
      ...
      expect(CalendarDate.fromISO(value)).toBeUndefined();
    }
  });
});
