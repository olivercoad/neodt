...
Unit suites share `test/helpers/adapters.ts`; configured entry-point tests and SSR share `test/helpers/entries.ts`. Both helpers derive their matrices from `libraries.ts` and the integration files. Common parser, keyboard, accessibility, controlled-state, calendar, and formatter-cache contracts run for every registered package implementation. Native Temporal runs in supporting browsers; its root entry is also exercised with an explicitly supplied global implementation in unit and SSR tests. Each integration declares its calendar expectations alongside its implementation, documenting native library differences instead of skipping whole adapters. Library-specific APIs (opaque Luxon zones, Date subclasses, non-Gregorian calendars) keep dedicated tests. Completion and placeholder animation helpers have no adapter dependency and run once.
