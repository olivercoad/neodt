...
for (const implementation of builtInAdapters)
  implementation.run(({ adapter, date, zone, iso, localValue }) => {
    describe(implementation.name, () => {
      const referenceTime = date("2026-04-15T12:00:00Z");

      function parse(input: string, zoneId = "UTC", locale?: Intl.LocalesArgument) {
        const value = parseNaturalDate(input, {
          adapter,
          referenceTime,
          zone: zone(zoneId),
          locale,
      ...
      describe("parseNaturalDate", () => {
        ...
        it("uses the locale date order for ambiguous short numeric dates", () => {
          expect(parse("8/4", "UTC", "en-US")).toBe("2026-08-04T00:00+00:00");
          expect(parse("8/4", "UTC", "en-GB")).toBe("2026-04-08T00:00+00:00");
          expect(parse("31.12.2027", "UTC", "de-DE")).toBe("2027-12-31T00:00+00:00");
        });
        ...
        it("resolves last weekdays to the previous occurrence", () => {
          const tuesday = date("2026-08-18T12:00:00Z");
          const parsed = parseNaturalDate("last wednesday", {
            adapter,
            referenceTime: tuesday,
            zone: zone("UTC"),
          });
          expect(parsed === undefined ? undefined : iso(parsed)).toBe("2026-08-12T00:00+00:00");
        });

        it("parses every rotating placeholder example", () => {
          for (const input of naturalTextExamples) {
            expect(
              parseNaturalDate(input, { adapter, referenceTime, zone: zone("UTC") }),
            ).toBeDefined();
          }
        });
      ...
      it("keeps incomplete and out-of-range text safe in the component's parser", () => {
        const referenceTime = calendarDate(adapter, date("2026-01-01T12:00Z"));
        for (const text of [
      ...
      it("asks the adapter for locale field order", () => {
        ...
        expect(localValue(result).slice(0, 10)).toBe("2026-04-08");
      });
    });
  });
