...
for (const implementation of builtInAdapters)
  implementation.run(({ adapter, date, localValue }) => {
    type Value = ReturnType<typeof date>;
    describe(implementation.name, () => {
      const DateTimeLocal = Neodt;

      const referenceTime = date("2026-08-17T15:30:00Z");
      ...
      describe("Neodt", () => {
        it("does not wrap when the widest idle locale value fits", async () => {
          ...
          await nextRender();
          expect(
            control.querySelector(".datetime-neo__content")?.hasAttribute("data-wrapped"),
          ).toBe(false);
          expect(
          ...
          expect(
            measurement.querySelector(".datetime-neo__editor .datetime-neo__value"),
          ).not.toBeNull();
          expect(
            measurement.querySelector(".datetime-neo__editor .datetime-neo__segment"),
          ).not.toBeNull();
          expect(
        ...
        it("locks a two-row layout from idle width across focus and natural entry", async () => {
          ...
          await nextRender();
          expect(
            (control.querySelector(".datetime-neo__content") as HTMLElement | null)?.dataset
              .wrapped,
          ).toBe("");
        ...
        it("formats years with fewer than four digits using leading zeros", () =>
          createRoot(() => {
            ...
            expect(
              control.querySelector<HTMLButtonElement>('[aria-label="year"]')?.textContent,
            ).toBe("0001");
          }));
        ...
        it("copies ISO date text unless all segments are selected", async () => {
          ...
          const isoDate = "2026-08-17T15:30Z";
          const normalCopy = copy(
            control.querySelector<HTMLButtonElement>(".datetime-neo__segment")!,
          );
        ...
        it("uses the reference timezone for editing and hides its offset by default", () =>
          createRoot(() => {
            const sydneyReference = date("2026-08-17T15:30:00", "Australia/Sydney");
            const control = (
              <DateTimeLocal
                adapter={adapter}
            ...
            expect(control.querySelector(".datetime-neo__segment")?.textContent).toBe("18");
        ...
        it("shows the selected offset including zero minutes when showTimeOffset is enabled", () =>
          createRoot(() => {
            ...
            expect(timezone?.parentElement?.classList.contains("datetime-neo__trailing")).toBe(
              true,
            );
        ...
        it("rounds offsets to minutes and keeps minutes in measurement layouts", () =>
          createRoot(() => {
            const fourHourReference = date("2026-08-17T15:30:00", "+04:00");
            ...
            expect(fourHourControl.querySelector(".datetime-neo__timezone")?.textContent).toBe(
              "+4:00",
            );
            expect(
              fourHourControl.querySelector(".datetime-neo__measurement .datetime-neo__timezone")
                ?.textContent,
            ).toBe(adapter.isOffsetFixed?.(fourHourReference) ? "+4:00" : "+88:88");

            const halfHourReference = date("2026-08-17T15:30:00", "+10:30");
            ...
            expect(halfHourControl.querySelector(".datetime-neo__timezone")?.textContent).toBe(
              "+10:30",
            );
            expect(
              halfHourControl.querySelector(".datetime-neo__measurement .datetime-neo__timezone")
                ?.textContent,
            ).toBe(adapter.isOffsetFixed?.(halfHourReference) ? "+10:30" : "+88:88");
          }));
        ...
        it("parses and confirms a natural-language date from the magic input", async () => {
          ...
          expect(
            control.querySelector('[aria-label="Cancel natural-language date"]'),
          ).not.toBeNull();
        ...
        it("advances the placeholder example when natural entry is reopened", async () => {
          ...
          const control = createRoot((rootDispose) => {
            dispose = rootDispose;
            return (
              <DateTimeLocal adapter={adapter} referenceTime={referenceTime} />
            ) as HTMLSpanElement;
          });
          document.body.append(control);
          const magicButton = control.querySelector<HTMLButtonElement>(
        ...
        it("opens natural-language entry with the @ key", async () => {
          ...
          expect(document.activeElement).toBe(
            control.querySelector(".datetime-neo__natural-input"),
          );
          control.remove();
          dispose!();
        });
        ...
        it("preserves a requested timezone name alongside the magic-mode offset", async () => {
          let dispose: (() => void) | undefined;
          const control = createRoot((rootDispose) => {
            dispose = rootDispose;
            return (
          ...
          input.dispatchEvent(new InputEvent("input", { bubbles: true }));
          await nextRender();
          expect(control.querySelector(".datetime-neo__natural-preview")?.textContent).toMatch(
            /GMT|UTC/,
          );
          expect(control.querySelector(".datetime-neo__timezone")?.textContent).toBe("+0:00");
          expect(
            control.querySelectorAll(
        ...
        it("cancels natural-language entry when no date can be parsed", async () => {
          ...
          control
            ...
            .click();
          await nextRender();
          control.querySelector<HTMLInputElement>(".datetime-neo__natural-input")!.value =
            "not a date";
          control
          ...
          await nextRender();
          control
            .querySelector<HTMLButtonElement>('[aria-label="Cancel natural-language date"]')!
            .click();
          await nextRender();
          expect(control.querySelector(".datetime-neo__natural-input")).toBeNull();
          expect(document.activeElement).toBe(control.querySelector(".datetime-neo__segment"));
        ...
        it("accepts the active natural-language completion with Tab", async () => {
          ...
          input.dispatchEvent(new InputEvent("input", { bubbles: true }));
          await nextRender();
          expect(control.querySelector(".datetime-neo__natural-ghost")?.textContent).toContain(
            "orrow",
          );
          key(input, "Tab");
          await nextRender();
        ...
        it("exits natural-language entry with Escape even when a completion is active", async () => {
          ...
          expect(control.querySelector(".datetime-neo__natural-input")).toBeNull();
          expect(
            control.querySelector('[aria-label="Enter date and time naturally"]'),
          ).not.toBeNull();
          control.remove();
          dispose!();
        });
        ...
        it("renders a date assigned after a controlled null value", async () => {
          let dispose: (() => void) | undefined;
          let setValue: (value: Value | null) => void;
          const control = createRoot((rootDispose) => {
            dispose = rootDispose;
            const [value, setControlledValue] = createSignal<Value | null>(null);
          ...
          expect(control.querySelector(".datetime-neo__value")?.textContent).toBe(
            "01/09/2026, 08:45",
          );
        ...
        it("selects focused segments and moves focus with left and right arrows", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              ...
              segments[1]?.focus();
              nextRender().then(() => {
                expect(segments[1]?.classList.contains("datetime-neo__segment--selected")).toBe(
                  true,
                );
                key(segments[1]!, "ArrowRight");
                nextRender().then(() => {
                  expect(document.activeElement).toBe(segments[2]);
                  expect(segments[2]?.classList.contains("datetime-neo__segment--selected")).toBe(
                    true,
                  );
          ...
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              ...
              nextRender().then(() => {
                ...
                expect(
                  control.querySelectorAll<HTMLButtonElement>('button[tabindex="0"]'),
                ).toHaveLength(1);
        ...
        it("focuses the first segment when the empty editor area is clicked", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              ...
              document.body.append(control);
              const emptyArea = control.querySelector<HTMLSpanElement>(
                ".datetime-neo__empty-area",
              )!;
              const firstSegment =
                control.querySelector<HTMLButtonElement>(".datetime-neo__segment")!;
              const segments = control.querySelectorAll<HTMLButtonElement>(
              ...
              nextRender().then(() => {
                ...
                expect(firstSegment.classList.contains("datetime-neo__segment--selected")).toBe(
                  true,
                );
                expect(
                  segments[segments.length - 1]?.classList.contains(
                    "datetime-neo__segment--selected",
                  ),
                ).toBe(false);
                control.remove();
                dispose();
        ...
        it.each(
          ...
          "restores a cleared $segment before adjusting with $arrow",
          async ({ segment, arrow }) => {
            const initialDate = date("2026-10-17T15:30");
            let dispose!: () => void;
            const [value, setValue] = createSignal<Value | null>(initialDate);
            const control = createRoot((rootDispose) => {
              dispose = rootDispose;
              return (
                <DateTimeLocal
        ...
        it("keeps focus while repeatedly adjusting a segment", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              const [value, setValue] = createSignal<Value>(date("2026-08-17T15:30"));
              const control = (
                <DateTimeLocal
                  adapter={adapter}
                  referenceTime={referenceTime}
        ...
        it("updates the displayed year after controlled numeric entry", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              const [value, setValue] = createSignal<Value | null>(date("2026-08-17T15:30"));
              const control = (
                <DateTimeLocal
                  adapter={adapter}
                  referenceTime={referenceTime}
        ...
        it("preserves a leading zero while entering an hour", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              ...
              document.body.append(control);
              const hour =
                control.querySelectorAll<HTMLButtonElement>(".datetime-neo__segment")[3]!;
              hour.focus();
              key(hour, "0");
              ...
              nextRender().then(() => {
                ...
                expect(document.activeElement).toBe(
                  control.querySelectorAll(".datetime-neo__segment")[4],
                );
                control.remove();
                dispose();
                resolve();
        ...
        it("sets PM when a 12-hour entry exceeds 12", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              const [value, setValue] = createSignal<Value>(date("2026-08-17T05:30"));
              const control = (
                <DateTimeLocal
                  adapter={adapter}
                  referenceTime={referenceTime}
              ...
              nextRender().then(() => {
                ...
                expect(
                  control.querySelector<HTMLButtonElement>('[aria-label="hour"]')?.textContent,
                ).toBe("3");
                expect(
        ...
        it("sets the draft day period to PM when the year is cleared", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              const [value, setValue] = createSignal<Value | null>(date("2026-08-17T05:30"));
              const control = (
                <DateTimeLocal
                  adapter={adapter}
                  referenceTime={referenceTime}
        ...
        it("allows day 31 when the month is cleared", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              const [value, setValue] = createSignal<Value | null>(date("2026-02-17T15:30"));
              const control = (
                <DateTimeLocal
                  adapter={adapter}
                  referenceTime={referenceTime}
        ...
        it("increments a cleared-month day to 31 instead of rolling into the next month", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              const [value, setValue] = createSignal<Value | null>(date("2026-04-30T15:30"));
              const control = (
                <DateTimeLocal
                  adapter={adapter}
                  referenceTime={referenceTime}
        ...
        it("rolls February 29 to March 1 and rejects February 30 when the year is cleared", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              const [value, setValue] = createSignal<Value | null>(date("2026-02-28T15:30"));
              const control = (
                <DateTimeLocal
                  adapter={adapter}
                  referenceTime={referenceTime}
        ...
        it("uses the nearest leap year for February day navigation when the year is cleared", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              const [value, setValue] = createSignal<Value | null>(date("2025-02-28T15:30"));
              const control = (
                <DateTimeLocal
                  adapter={adapter}
                  referenceTime={referenceTime}
              ...
              nextRender().then(() => {
                ...
                nextRender().then(() => {
                  ...
                  nextRender().then(() => {
                    expect(day.textContent).toBe("29");
                    expect(
                      control.querySelector<HTMLInputElement>('input[type="datetime-local"]')
                        ?.value,
                    ).toBe("2024-02-29T15:30");
                    control.remove();
                    dispose();
          ...
          async ({ locale, controlled }) => {
            ...
            try {
              ...
              for (const index of [2, 1]) {
                ...
                expect(
                  segments[index - 1]?.querySelector(".datetime-neo__placeholder"),
                ).not.toBeNull();
                expect(control.querySelectorAll(".datetime-neo__placeholder")).toHaveLength(
                  4 - index,
                );
        ...
        it("clears individual segments with backspace or delete and renders contextual placeholders", async () =>
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              ...
              document.body.append(control);
              const segments =
                control.querySelectorAll<HTMLButtonElement>(".datetime-neo__segment");
              segments[0]!.focus();
              key(segments[0]!, "Backspace");
              nextRender().then(() => {
                expect(value()).toBeNull();
            ...
            createRoot((dispose) => {
              ...
              nextRender().then(() => {
                ...
                nextRender().then(() => {
                  ...
                  nextRender().then(() => {
                    expect(localValue(onValueChange.mock.calls.at(-1)?.[0])).toBe(
                      "2063-08-17T15:30",
                    );
                    control.remove();
                    dispose();
          ...
          await new Promise<void>((resolve) =>
            createRoot((dispose) => {
              ...
              nextRender().then(() => {
                ...
                key(
                  readonly.querySelectorAll<HTMLButtonElement>(".datetime-neo__segment")[0]!,
                  "ArrowUp",
                );
                expect(
                  readonly.querySelectorAll<HTMLButtonElement>(".datetime-neo__segment")[0]
                ...
                const readonlySegment =
                  readonly.querySelector<HTMLElement>(".datetime-neo__segment")!;
                ...
                const disabledSegment =
                  disabled.querySelector<HTMLElement>(".datetime-neo__segment")!;
