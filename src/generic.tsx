...
/** The adapter determines the type of every input value and callback. */
export interface NeodtProps<T, TZone = string> extends JSX.HTMLAttributes<HTMLSpanElement> {
...
type InternalProps = Omit<NeodtProps<DateTime>, "adapter">;

function Neodt<T, TZone>(props: NeodtProps<T, TZone>): JSX.Element {
  ...
  return (
    <NeodtControl
      {...rest}
      referenceTime={convert(local.referenceTime)}
      value={
        local.value === undefined ? undefined : local.value === null ? null : convert(local.value)
      }
      defaultValue={local.defaultValue === undefined ? undefined : convert(local.defaultValue)}
      onValueChange={(value) =>
        local.onValueChange?.(
          value === null
            ? null
            : local.adapter.fromEpochMilliseconds(
                value.milliseconds,
                local.adapter.getZone(local.referenceTime),
              ),
        )
      }
    />
...
function NeodtControl(props: InternalProps): JSX.Element {
  const [local, rest] = splitProps(props, [
    "referenceTime",
    "value",
    "defaultValue",
