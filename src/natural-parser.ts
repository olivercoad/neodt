...
const months: Record<string, number> = {
  january: 1, jan: 1, j: 1, february: 2, feb: 2, febuary: 2, f: 2, march: 3, mar: 3, m: 3,
...
const aliases: Record<string, string> = { tomorow: 'tomorrow', tmrw: 'tomorrow', tmrrw: 'tomorrow', '2mrw': 'tomorrow', pluz: 'plus', wks: 'weeks', wk: 'week', hrs: 'hours', hr: 'hour' }

type Clock = { hour: number; minute: number }

/** Parses one natural-language date-time. Deliberately does not parse date ranges. */
export function parseNaturalDate(value: string, options: NaturalDateParseOptions): DateTime | undefined {
  ...
  const zoned = value.trim().match(/^(.*?)(?:\s+(America\/[\w-]+|Europe\/[\w-]+|Australia\/[\w-]+|Asia\/[\w-]+|UTC))$/i)
  if (zoned) {
  ...
  const relative = input.match(/^in\s+(.+)$|^(.+)\s+ago$|^(.+)\s+in the past$|^(.+?)\s+from\s+(.+)$/)
  if (relative) {
    ...
    return base && duration ? base.plus(scaleDuration(duration, direction)).startOf('minute') : undefined
  }
...
function normalize(value: string): string {
  return value.trim().toLowerCase().replace(/[’']/g, '').replace(/[,]/g, ' ').replace(/\s+/g, ' ').split(' ').map(token => aliases[token] ?? token).join(' ')
}

function splitClock(input: string): { date: string; clock?: Clock } {
  const match = input.match(/^(.*?)(?:\s+(?:at\s+)?((?:\d{1,2}(?::\d{2})\s*(?:am|pm)?|\d{1,2}\s*(?:am|pm))|noon|midnight))$/)
...
function parseClock(value: string): Clock | undefined {
  ...
  if (minute > 59 || hour > 23 || (!match[3] && hour > 23) || (match[3] && (hour < 1 || hour > 12))) return undefined
  if (match[3] === 'pm' && hour !== 12) hour += 12
  if (match[3] === 'am' && hour === 12) hour = 0
  return { hour, minute }
}

function parseDate(input: string, now: DateTime, locale?: Intl.LocalesArgument): DateTime | undefined {
  const dayWords: Record<string, number> = { today: 0, tomorrow: 1, yesterday: -1, 'day after tomorrow': 2, 'day before yesterday': -2, 'the day after tomorrow': 2, 'the day before yesterday': -2 }
  ...
  if (weekdayToken && weekdays[weekdayToken]) {
    const wanted = weekdays[weekdayToken]!; const direction = weekday[1] === 'last' ? -1 : 1
  ...
  const holiday = resolveHoliday(input, now)
  ...
  if (numeric) {
    ...
    return parts ? makeDate(now, numeric[3] ? normalizeYear(numeric[3], now.year) : now.year, parts[0], parts[1]) : undefined
  }
  const textual = input.match(/^([a-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?(?:\s+(\d{2,4}))?$/)
  const monthToken = textual?.[1]
  if (textual && monthToken && months[monthToken]) return makeDate(now, textual[3] ? normalizeYear(textual[3], now.year) : now.year, months[monthToken]!, Number(textual[2]))
  const reverseTextual = input.match(/^(\d{1,2})(?:st|nd|rd|th)?\s+([a-z]+)(?:\s+(\d{2,4}))?$/)
  const reverseMonthToken = reverseTextual?.[2]
  if (reverseTextual && reverseMonthToken && months[reverseMonthToken]) return makeDate(now, reverseTextual[3] ? normalizeYear(reverseTextual[3], now.year) : now.year, months[reverseMonthToken]!, Number(reverseTextual[1]))
  const ordinal = input.match(/^(?:the\s+)?(\d{1,2})(?:st|nd|rd|th)$/)
  if (ordinal) {
    const candidate = makeDate(now, now.year, now.month, Number(ordinal[1]))
    return candidate && candidate < now.startOf('day') ? candidate.plus({ months: 1 }).startOf('month').set({ day: Number(ordinal[1]) }) : candidate
  }
  return undefined
}

function numericDateParts(first: number, second: number, locale?: Intl.LocalesArgument): [number, number] | undefined {
...
function localeUsesDayFirst(locale: Intl.LocalesArgument | undefined): boolean {
  const parts = new Intl.DateTimeFormat(locale ?? 'en-US', { day: 'numeric', month: 'numeric', year: 'numeric' }).formatToParts(new Date(Date.UTC(2001, 1, 3)))
...
function normalizeYear(raw: string, referenceYear: number): number {
  ...
  return candidate - referenceYear > 50 ? candidate - 100 : referenceYear - candidate > 50 ? candidate + 100 : candidate
}

function applyClock(date: DateTime, clock?: Clock): DateTime { return clock ? date.set(clock) : date }
function parseDuration(value: string): Partial<Record<DurationUnit, number>> | undefined {
  const match = value.trim().match(/^(a|an|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|\d+)\s*(minute|minutes|hour|hours|day|days|week|weeks|month|months|year|years)?$/)
  if (!match) return undefined
  const amount = counts[match[1]!] ?? Number(match[1]!); const unit = (match[2] ?? 'days').replace(/s$/, '') as DurationUnit
  return Number.isFinite(amount) ? { [unit]: amount } : undefined
}
function scaleDuration(duration: Partial<Record<DurationUnit, number>>, factor: number): Partial<Record<DurationUnit, number>> { return Object.fromEntries(Object.entries(duration).map(([unit, amount]) => [unit, amount * factor])) as Partial<Record<DurationUnit, number>> }

function resolveHoliday(input: string, now: DateTime): DateTime | undefined {
  const annual: Record<string, [number, number]> = { christmas: [12, 25], xmas: [12, 25], 'christmas eve': [12, 24], halloween: [10, 31], 'new years': [1, 1], 'new years day': [1, 1], 'new year': [1, 1], 'new years eve': [12, 31], nye: [12, 31], 'valentines day': [2, 14], valentines: [2, 14], 'independence day': [7, 4], '4th of july': [7, 4], 'july 4th': [7, 4] }
  const annualDate = annual[input]
  if (annualDate) { const [month, day] = annualDate; const date = makeDate(now, now.year, month, day); return date && date < now.startOf('day') ? date.plus({ years: 1 }) : date }
  if (input === 'labor day') return nthWeekday(now, 9, 1, 1)
  if (input === 'thanksgiving' || input === 'thanksgiving day') return nthWeekday(now, 11, 4, 4)
  if (input === 'mothers day') return nthWeekday(now, 5, 7, 2)
  if (input === 'fathers day') return nthWeekday(now, 6, 7, 3)
  if (input === 'memorial day') { const end = makeDate(now, now.year, 5, 31)!; const date = end.minus({ days: (end.weekday - 1 + 7) % 7 }); return date < now.startOf('day') ? date.plus({ years: 1 }) : date }
  return undefined
}
function nthWeekday(now: DateTime, month: number, weekday: number, occurrence: number): DateTime { let date = makeDate(now, now.year, month, 1)!; date = date.plus({ days: (weekday - date.weekday + 7) % 7 + 7 * (occurrence - 1) }); return date < now.startOf('day') ? nthWeekday(now.plus({ years: 1 }), month, weekday, occurrence) : date }
